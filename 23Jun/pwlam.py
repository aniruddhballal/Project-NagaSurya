...
# Define a dictionary with lambda functions and their corresponding names
functions = {
    'sin(y)': lambda y, x: np.sin(y),
    'sin(x)': lambda y, x: np.sin(x),
    'cos(y)': lambda y, x: np.cos(y),
    'cos(x)': lambda y, x: np.cos(x)
}

# Choose the function to use
func_name = 'sin(y)'
b = functions[func_name]
...
def adaptive_calcalm(b, lmax, num_points_theta, num_points_phi, func_name):
    alm = {}

    csv_filename = f'theta{num_points_theta}, phi{num_points_phi}, lmax{lmax}, {func_name}.csv'
...
def plotty01(ax, x, y, bvals, type, lmax, num_points_theta, num_points_phi, contnum, func_name):
    ...
    name = ''
    if type == 1:
        name = f'og function ({func_name})'
    elif type == 2:
        name = f'recons (lmax: {lmax}, theta: {num_points_theta}, phi: {num_points_phi}, b: {func_name})'
    elif type == 3:
        name = 'Delta'
    ax.set_title(name)
    ax.set_xticks(np.linspace(0, 2 * np.pi, 5))
...
plotty01(axs[0], x, y, b(y, x), 1, 5, num_points_theta, num_points_phi, contnum, func_name)

alm = adaptive_calcalm(b, lmax, num_points_theta, num_points_phi, func_name)
x = np.linspace(0, 2 * np.pi, num_points_phi)
y = np.linspace(0, np.pi, num_points_theta)
y, x = np.meshgrid(y, x)
b_recons = recons(alm, x, y, lmax)
plotty01(axs[1], x, y, b_recons, 2, lmax, num_points_theta, num_points_phi, contnum, func_name)
