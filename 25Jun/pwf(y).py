...
def adaptive_calcalm(b, lmax, num_points_theta, num_points_phi):
    ...
    csv_filename = os.path.join(csv_folder, f'theta{num_points_theta}, phi{num_points_phi}, lmax{lmax}, y_minus_3cos_0.4y2.csv')
...
def plotty(ax, x, y, bvals, type, lmax, num_points_theta, num_points_phi, contnum):
    ...
    if type == 1:
        ...
        name = f'recons (lmax: {lmax}, theta: {num_points_theta}, phi: {num_points_phi}, b: y - 3cos(0.4y^2))'
    elif type == 3:
        name = 'Delta'
    ax.set_title(name)
...
plotty(axs[2], x, y, delta, 3, lmax, num_points_theta, num_points_phi, contnum)
