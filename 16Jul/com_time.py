...
def process_all_csv_files():
    ...
    for csv_file in os.listdir(alm_folder):
        if csv_file.endswith('.csv'):
            ...
            center_of_mass_l = plot_and_save(l_values, m_values, alm_magnitudes, map_number, map_date, threshold, max_alm)
    ...
    ax1.plot(date_list, smoothed_center_of_mass_l, label='Smoothed Center of Mass l', marker='o', color='b')
    ax1.set_xlabel('Year', fontsize=20)
    ax1.set_ylabel('Center of Mass', fontsize=20, color='b')
    ...
    fig.suptitle('Smoothed Center of Mass and Magnetic Flux over Time', fontsize=18)
    fig.legend(loc='upper left', fontsize=15)
    fig.tight_layout()  # Adjust layout to make room for the legend

    plt.xticks(rotation=45)  # Rotate x-axis labels for better visibility

    pltfolder = 'comvstime plots'
    os.makedirs(pltfolder, exist_ok=True)
    plot_filename = os.path.join(pltfolder, 'center_of_mass_and_flux_over_time.png')
