...
def process_all_csv_files():
    ...
    for csv_file in os.listdir(folder_name):
        if csv_file.endswith('.csv'):
            ...
            # Plot spherical harmonics in subplot 4
            ax = fig.add_subplot(224, projection='3d')

            # Specify the l and m values
            l = lmax  # Use lmax as l
            m = m_max  # Use m_max as m
            ...
            # Add colorbar
            cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=colormap), ax=ax, shrink=0.8)
            cbar.ax.tick_params(labelsize=8)

            plotty200(axs[1, 0], x, y, b)

            # Accessing axs[1, 1] to modify its properties
            ax = axs[1, 1]

            # Hide the spines (edges)
