...
def plotty(ax, x, y, bvals, carrington_map_number):
    ...
    name = f'dipole ({vmax:.2f} to {vmin:.2f}) - CR map: {carrington_map_number} {month_year}'
...
def process_carrington_map(carrington_map_number, lmax):
    ...
    fits_file = f'E:/my_env1/we goin solar/11Jul/dipole stuff/fits_files/hmi.Synoptic_Mr_small.{carrington_map_number}.fits'
    ...
    # Reconstruct the Carrington map
    b_reconstructed = recons(alm, x, y, lmax, verbose=True)  # Pass verbose=True to show progress bar

    # Plot only the reconstructed data using plotty function
    fig, ax = plt.subplots(figsize=(12, 6))  # 2:1 aspect ratio
...
fits_folder = 'E:/my_env1/we goin solar/11Jul/dipole stuff/fits_files'
fits_files = os.listdir(fits_folder)

done_folder = os.path.join(fits_folder, 'done')# Create the "done" folder if it doesn't exist
if not os.path.exists(done_folder):
    os.makedirs(done_folder)

for fits_filename in fits_files:
    # Check if the filename matches the pattern "hmi.Synoptic_Mr_small.xxxx.fits"
    if fits_filename.startswith('hmi.Synoptic_Mr_small.') and fits_filename.endswith('.fits'):
        # Extract the Carrington map number from the filename
        carrington_map_number = int(fits_filename.split('.')[2])

        # Update fits_file path based on the Carrington map number
        ...
        try:
            ...
            # Continue with the rest of your process_carrington_map function as before
            process_carrington_map(carrington_map_number, lmax)

            # Move the FITS file to the "done" folder
