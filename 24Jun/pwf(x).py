...
def adaptive_calcalm(b, lmax, num_points_theta, num_points_phi):
    ...
    csv_filename = os.path.join(csv_folder, f'theta{num_points_theta}, phi{num_points_phi}, lmax{lmax}, x_minus_3cos_0.4x2.csv')
...
def plottyminmax(ax, x, y, bvals, type, lmax, num_points_theta, num_points_phi, contnum):
    ...
    name = ''
    if type == 1:
        name = 'og function (x - 3cos(0.4x^2))'
    elif type == 2:
...
def plotty01(ax, x, y, bvals, type, lmax, num_points_theta, num_points_phi, contnum):
    ...
    if type == 1:
        ...
        name = f'recons (lmax: {lmax}, theta: {num_points_theta}, phi: {num_points_phi}, b: x - 3cos(0.4x^2))'
    elif type == 3:
        name = 'Delta'
    ax.set_title(name)
    ax.set_xticks(np.linspace(0, 2 * np.pi, 5))
...
error_folder = 'total abs errors - lmax 20 f(x)'
...
csv_filename = os.path.join(csv_folder, f'theta{num_points_theta}, phi{num_points_phi}, lmax{lmax}, f(x).csv')
