...
def plottyminmax(ax, x, y, bvals, type, lmax, num_points_theta, num_points_phi, contnum):
    ...
    name = ''
    if type == 1:
        name = 'og function (y - 3cos(0.4y^2))'
    elif type == 2:
...
error_folder = 'total abs errors - lmax 20 f(y)'
...
csv_filename = os.path.join(csv_folder, f'theta{num_points_theta}, phi{num_points_phi}, lmax{lmax}, f(y).csv')
