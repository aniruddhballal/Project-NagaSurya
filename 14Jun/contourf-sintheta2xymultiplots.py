...
# Define parameters for different subplots
lmax_values = [1, 5, 10]
num_points_values = [10, 50, 100]
...
y, x = np.meshgrid(y, x)
plotty(axs[0, 0], x, y, b(y, x), 1, None, None)

# Reconstructed plots
for i, (lmax, num_points) in enumerate(zip(lmax_values, num_points_values)):
    ...
    row = (i + 1) // 2
    col = (i + 1) % 2
    plotty(axs[row, col], x, y, b_reconstructed, 2, lmax, num_points)
