...
def recons(alm, x, y, lmax, sigma=2):
    sizex = x.shape
    ...
    for l in range(lmax + 1):
        for m in range(-l, l + 1):
            ...
            brecons += alm[(l, m)] * ylm
    # Apply Gaussian filter to smooth the reconstructed data
    brecons_real = brecons.real
    brecons_smoothed = gaussian_filter(brecons_real, sigma=sigma)
    return brecons_smoothed
...
def plotty(ax, x, y, bvals, type, lmax, num_points_theta, num_points_phi, contnum, bname):
    ...
    if type == 1:
        ...
        name = f'recons with filter (lmax: {lmax}, theta: {num_points_theta}, phi: {num_points_phi})'
    elif type == 3:
        name = 'Delta'
    ax.set_title(name)
...
y = np.linspace(0, np.pi, num_points_theta)
y, x = np.meshgrid(y, x)
b_reconstructed = recons(alm, x, y, lmax, sigma=2)
