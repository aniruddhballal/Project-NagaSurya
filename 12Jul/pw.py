...
def plot_and_save(l_values, m_values, alm_magnitudes, map_number, month_year, threshold, max_alm, ax):
    ...
    cbar.set_label(r'Magnitude of alm')
    ...
    ax.scatter(l_max, m_max, c='red', marker='s', s=50, label=f'Max alm: {alm_magnitudes_filtered[max_index]:.2f}')
    ax.scatter(l_second_max, m_second_max, c='blue', marker='s', s=50, label=f'Second Max alm: {alm_magnitudes_filtered[second_max_index]:.2f}')
...
def plot_sphere(ax, l, m, colormap='coolwarm'):
    ...
    # Set plot title
    ax.set_title(r'$Y_{%d}^{%d}$ but spherical' % (l, m), fontsize = 16)
...
def process_all_csv_files():
    ...
    for csv_file in os.listdir(folder_name):
        if csv_file.endswith('.csv'):
            ...
            ax_left = fig.add_axes([0.405, 0.1, 0.3, 0.3], projection='3d')  # Adjust the left subplot size and position
            plot_atomic(ax_left, ls[0], ms[0], 'coolwarm_r')

            # Plot the second ylm on the right side
            ax_right = fig.add_axes([0.53, 0.1, 0.3, 0.3], projection='3d')  # Adjust the right subplot size and position
            plot_sphere(ax_right, ls[0], ms[0], 'coolwarm_r')

            # Plot the first ylm on the left side
            ax_left = fig.add_axes([0.655, 0.1, 0.3, 0.3], projection='3d')  # Adjust the left subplot size and position
            plot_atomic(ax_left, ls[1], ms[1], 'coolwarm_r')

            # Plot the second ylm on the right side
            ax_right = fig.add_axes([0.78, 0.1, 0.3, 0.3], projection='3d')  # Adjust the right subplot size and position
